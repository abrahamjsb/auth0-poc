...
var options = {
  url: "https://abrahamjsb.us.auth0.com/oauth/token",
  headers: { "content-type": "application/json" },
  data: '{"client_id":"","client_secret":"","audience":"https://abrahamjsb.us.auth0.com/api/v2/","grant_type":"client_credentials"}',
};
...
async function requestManagementAPIAccessToken() {
  try {
    const request = await fetch(options.url, {
      body: '{"client_id":"","client_secret":"","audience":"https://abrahamjsb.us.auth0.com/api/v2/","grant_type":"client_credentials"}',
      method: "POST",
      headers: { "Content-Type": "application/json" },
    });
...
app.get("/api/factors", checkJwt, async (req, res) => {
  try {
    ...
    const requestFactors = await fetch(
      `${issuerBaseUrl}/api/v2/guardian/factors`,
      {
...
app.post("/api/factors/enrollment", checkJwt, async (req, res) => {
  try {
    const accessToken = await requestManagementAPIAccessToken();
    const requestFactorEnrollTicker = await fetch(
      `${issuerBaseUrl}/api/v2/guardian/enrollments/ticket`,
      {
        headers: {
...
app.delete(
  "api/factors/:factorMethod/delete/:user",
  checkJwt,
  async (req, res) => {
    try {
      ...
      const accessToken = await requestManagementAPIAccessToken();
      await fetch(
        `${issuerBaseUrl}/api/v2/users/${userId}/authentication-methods/${factorMethod}`,
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
      );
...
app.post(
  "api/factors/invalidate-browsers/:userId",
  checkJwt,
  async (req, res) => {
    try {
